...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_scrolling__WEBPACK_IMPORTED_MODULE_12__["default"])('.pageup', 'up');
  document.querySelectorAll('.header-menu-sub li').forEach(function (item) {
    Object(_modules_scrolling__WEBPACK_IMPORTED_MODULE_12__["default"])('.' + item.className);
  });
});
...
var mask = function mask(selector) {
  var setCursorPosition = function setCursorPosition(pos, elem) {
    ...
    if (elem.setSelectionRange) {
      ...
      var range = elem.createTextRange(); //объединение начала иа конца диапазона
...
var modals = function modals() {
  ...
  var giftMarginChangeonClose = function giftMarginChangeonClose() {
    var gift = document.querySelector('.fixed-gift');

    if (gift) {
      var scroll = +calcScroll();
      var wr = +getComputedStyle(gift).right.replace(/[\D]/g, '');
      gift.style.right = "".concat(wr - scroll, "px");
    }
  };

  var giftMarginChangeonOpen = function giftMarginChangeonOpen() {
    var gift = document.querySelector('.fixed-gift');
...
var scrolling = function scrolling(upSelector, up) {
...
var showMoreStyles = function showMoreStyles(trigger, wrapper) {
  ...
  btn.addEventListener('click', function (e) {
    Object(_services_requests__WEBPACK_IMPORTED_MODULE_3__["getResourse"])('./assets/db.json').then(function (res) {
      return createCards(res.styles, e.target);
    }).catch(function (error) {
      return console.log(error);
    });
  });

  function createCards(response, target) {
    response.forEach(function (_ref) {
      var src = _ref.src,
          title = _ref.title,
          link = _ref.link;
      var card = document.createElement('div');
      card.classList.add('animated', 'faster', 'fadeInUp', 'col-sm-3', 'col-sm-offset-0', 'col-xs-10', 'col-xs-offset-1');
...
var sliders = function sliders(slides, dir, prev, next) {
  var slideIndex = 1;
  var paused = false; //флаг остановки слайдера 
  ...
  ;
  activateAnimation(); //очистка интервала при наведеннии мыши
    ...
    while (1) {
      switch (_context2.prev = _context2.next) {
        ...
        case 2:
          ...
          throw new Error("Could not ".concat(url, ", status ").concat(res.status));
